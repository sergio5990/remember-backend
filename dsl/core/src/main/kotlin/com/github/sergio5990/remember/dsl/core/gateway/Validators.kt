...
interface ContextValidator : Validator<GatewayContext> {
  override fun validate(context: GatewayContext?): List<ApiError>
}

class RoleValidator(private val roles: List<RoleName>) : ContextValidator {

  override fun validate(context: GatewayContext?): List<ApiError> {
    return if (context!!.user.roles.any { roles.contains(it) }) {
      errors()
    } else {
      errors("user doesn't have roles $roles")
    }
  }
}
